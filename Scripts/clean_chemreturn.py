...
raw_folder = r'F:\PestReturns\Raw'
cleaned_folder = r'F:\PestReturns\cleaned'

def clean_chemreturn(file_path,output_path):
    print(f"cleaning: {os.path.basename(file_path)}")

    df = pd.read_excel(file_path , skiprows = 9)
    df.columns = df.columns.str.strip()
    df = df.drop(['Voucher Ref. No.','Voucher Ref. Date','Round Off','Stock Return','CGST','SGST'],axis = 1)
    ...
    df=df.drop(['Particulars','Value','Gross Total','Pesticides Sales','CD on Pesticides Sales'], axis = 1)
    df['category']= 'chemreturn'
...
for file in os.listdir(raw_folder):
    if file.endswith('.xlsx'):
        ...
        clean_chemreturn(source_path, cleaned_path)

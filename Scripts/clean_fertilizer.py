...
raw_folder = r'F:\BioStimulants\Raw'
cleaned_folder = r'F:\BioStimulants\cleaned'

def clean_fertilizer(file_path,output_path):
    ...
    df = df.drop(['Gross Total','Sales','Cash Discount','Particulars','Pesticides Sales','GST Sales (Branches)'],axis = 1)
    df=df.dropna(subset = 'products')
    df = df.drop('Value',axis = 1)
    df.reset_index(drop=True,inplace = True)
    df['category']= 'Fertilizer'
...
for file in os.listdir(raw_folder):
    if file.endswith('.xlsx'):
        ...
        clean_fertilizer(source_path, cleaned_path)
